    ...
    public class GameView
    {
        public static Embed Info(PendingGame game)
        {
            IEnumerable<string> description = new[] {
                $"Attar has asked the broker to sell all of his {GameConfiguration.NumberOfRounds} {game.Collectable.Name}s. If he succeeds he'll get a bonus.",
                "Broker invites interested collectors to join free of charge.",
                $"You the {game.Collectable.Title}, who is missing {GameConfiguration.RoundsToVictory} {game.Collectable.Name}s from their most prized collection attend the auction with a budget of {GameConfiguration.GameCoins} coins.",
                $"Broker will auction {game.Collectable.Name}s one by one hoping to earn the bonus.",
            };

            IEnumerable<string> rules = new[]
            {
                $"You have {GameConfiguration.GameCoins} coins to spend.",
                $"Purchase {GameConfiguration.RoundsToVictory} {game.Collectable.Name}s and complete your collection.",
            ...
            return new EmbedBuilder()
                .WithTitle($"{game.Collectable.Emote}   A U C T I O N    G A M E   {game.Collectable.Emote}")
